...
class CustomUserAdmin(admin.ModelAdmin):
    list_display = ['id','email', 'user_type', 'is_active', 'is_staff']
    list_filter = ['user_type', 'is_active', 'is_staff']
    search_fields = ['email']

# The decorator above does the same thing as the following:
# admin.site.register(CustomUser, CustomUserAdmin)

class UniversityAdmin(admin.ModelAdmin):
    list_display = ['serial_no', 'university','name', 'concentration', 'website_url', 'campus', 'country', 'study_level', 'duration', 'intakes', 'entry_requirements', 'ielts_score',
                    ...
                    'scholarship_available','scholarship_detail','backlog_range', 'remarks', 'esl_elp_detail', 'application_mode']
...
class StudentAdmin(admin.ModelAdmin):
    list_display = ('id','name', 'family_name', 'email','region','preferred_language','contact_number','country_of_residence','highest_education','how_did_you_hear','status','ApplicationStage')
...
class AcademicAchievementAdmin(admin.ModelAdmin):
    list_display = ('id','highest_education', 'country_where_study_completed')

@admin.register(StudyPreference)
class StudyPreferenceAdmin(admin.ModelAdmin):
    list_display = ('id','intended_area_of_study', 'intended_course_level', 'courses_and_fields_comments','career_paths',
...
class PersonalDetailsAdmin(admin.ModelAdmin):
    list_display = ('id','address_1','address_2', 'city','state_province','country','postcode_zipcode', 
